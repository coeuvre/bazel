...
public class RxRemoteRetrier {
  ...
  public Publisher<Long> retryWhen(Flowable<Throwable> errors, Backoff backoff) {
    return Flowable.defer(() -> {
      ...
      return errors.flatMap(error -> {
        ...
        } else if (error instanceof Exception && isRetriable((Exception) error)) {
          long waitMillis = backoff.nextDelayMillis((Exception) error);
          if (waitMillis >= 0) {
  ...
  public boolean isRetriable(Exception t) {
